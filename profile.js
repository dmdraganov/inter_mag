...
function renderOrderHistory() {
  ...
  if (orders.length === 0) {
    ...
    orderHistory.innerHTML =
      '<p class="empty-message">История заказов пуста</p>'; // Сообщение
    return; // Выходим
  }
...
function renderProfileFavorites() {
  ...
  if (favoriteProducts.length === 0) {
    ...
    profileFavorites.innerHTML =
      '<p class="empty-message">Нет избранных товаров</p>'; // Сообщение
    return; // Выходим
  }
  ...
  profileFavorites.innerHTML = favoriteProducts
    .slice(0, 4)
    .map(
      (product) => `
