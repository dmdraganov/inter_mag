...
function renderOrderSummary() {
  ...
  document.getElementById("summarySubtotal").textContent =
    subtotal.toLocaleString("ru-RU") + " ₽"; // Подитог
  document.getElementById("summaryTotal").textContent =
    subtotal.toLocaleString("ru-RU") + " ₽"; // Итог
...
function handleCheckout(e) {
  ...
  cart.forEach((item) => {
    ...
    if (productIndex !== -1) {
      ...
      currentProducts[productIndex].stock =
        (currentProducts[productIndex].stock || 0) - item.quantity; // Уменьшаем
    }
  }); // Конец перебора
  ...
  alert(
    `Спасибо за заказ, ${order.customer.fullName}!\n\nЗаказ №${order.id}\nСумма: ${order.total.toLocaleString("ru-RU")} ₽\n\nМы свяжемся с вами по телефону ${order.customer.phone} для подтверждения.`,
  ); // Уведомление
